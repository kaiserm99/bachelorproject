...
# 	./get_models.sh "PROGRAM" --> ./get_models.sh "Impl(a, b), Impl(b, a)"
...
if [ -n "$1" ];
	then python3 stable_modells.py --minimal -p "$1" > $tmp_file;

	else python3 stable_modells.py --minimal > $tmp_file;
fi
...
./../Exercise_01_01/all_modells.sh $tmp_cnf $tmp_modells > $tmp_atoms  # This is just used so there is no ugly output
...
for number in $(tail -n +2 $tmp_modells)
do
	if [ "$number" -eq "0" ]; then
		echo 
   		continue
